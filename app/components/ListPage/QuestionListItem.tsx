...
export default function QuestionListItem(props: QuestionListItemProps) {
  ...
  return (
    <article className="flex items-stretch w-full space-x-4 min-h-[5.2rem]">
      ...
      <Link to={`./${props.id}`} className="w-full">
        <div className="flex flex-col h-full justify-between">
          <div>
            <h2 className="font-medium text-base leading-140% text-slate-600 line-clamp-1 max-w-[12.5rem]">
              {props.question}
            </h2>
            <p className="text-xs mt-1 leading-tight line-clamp-2">{props.question}</p>
          </div>

          <footer>
            <p className="text-slate-400 text-xs flex items-center">
              <span className="sr-only">Posted on</span>
              <time dateTime={parseDate(props.createdAt)}>{parseDate(props.createdAt, "DD MMMM HH:mm")}</time>
